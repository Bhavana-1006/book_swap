    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleShare}
            className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-cream-300 bg-white hover:bg-cream-100 text-xs font-semibold text-navy-900 transition-colors shadow-sm"
          ...
          {isOwner && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-cream-300 bg-white hover:bg-cream-100 text-xs font-bold text-navy-900 transition-colors shadow-sm"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <h1 className="font-serif text-3xl sm:text-4xl font-extrabold text-navy-900 dark:text-white leading-tight">
              {book.title}
            </h1>

            <p className="text-base text-gray-600 dark:text-slate-300 font-medium">
              By <span className="text-navy-900 dark:text-amber-400 font-bold">{book.author}</span>
            </p>
          </div>

          {/* Price Callout Banner in Indian Rupees */}
          <div className="p-5 rounded-2xl bg-gradient-to-r from-cream-100/90 via-cream-50 to-white dark:from-[#132B32] dark:via-[#16343C] dark:to-[#183B45] border border-cream-200 dark:border-[#26525E] shadow-soft flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Listing Price</p>
              <div className="flex items-center gap-2 mt-1">
                {book.listingType === 'SELL' ? (
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-3xl sm:text-4xl font-black text-navy-900 dark:text-white font-serif">
                      {formatPrice(book.price, 'SELL')}
                    </span>
                    <span className="text-xs font-bold text-gray-500 dark:text-slate-400">INR</span>
                  </div>
                ) : book.listingType === 'DONATE' ? (
                  <span className="text-2xl sm:text-3xl font-black text-emerald-700 dark:text-emerald-400 flex items-center gap-1.5">
                    <Sparkles className="w-6 h-6 text-amber-500" />
                    <span>Free Donation</span>
                  </span>
                ) : (
                  <span className="text-2xl sm:text-3xl font-black text-accent-700 dark:text-amber-400 flex items-center gap-1.5">
            ...
            <div className="text-right">
              <p className="text-xs text-gray-500 dark:text-slate-400 font-medium">Availability</p>
              <span
          ...
          {topicData && topicData.topics && topicData.topics.length > 0 && (
            <div className="p-5 rounded-2xl bg-white dark:bg-[#132B32] border border-cream-300 dark:border-[#26525E] shadow-soft space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <BookmarkCheck className="w-4 h-4 text-brand-600 dark:text-amber-400" />
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 ${
                        isSelected
                          ? 'bg-navy-900 dark:bg-amber-500 text-white dark:text-navy-950 shadow-sm scale-105'
                          : 'bg-cream-100 dark:bg-[#0E2328] hover:bg-cream-200 dark:hover:bg-[#183B45] text-navy-800 dark:text-slate-200 border border-cream-200 dark:border-[#26525E]'
                      }`}
                    >
                      <span>{t.name}</span>
                    </button>
              ...
              {topicData.topics[selectedTopicIndex] && (
                <div className="p-3.5 rounded-xl bg-cream-50 dark:bg-[#0E2328] border border-cream-200 dark:border-[#26525E] text-xs animate-slide-in">
                  <p className="font-bold text-navy-900 dark:text-amber-400 mb-1">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 rounded-2xl bg-cream-100/60 dark:bg-[#0E2328] border border-cream-200 dark:border-[#26525E] text-xs">
            {book.category && (
              <div className="space-y-0.5">
                <span className="text-gray-500 dark:text-slate-400 font-semibold">Category</span>
                <p className="text-navy-900 dark:text-white font-bold">{book.category}</p>
              </div>
            )}
            {book.branch && (
              <div className="space-y-0.5">
                <span className="text-gray-500 dark:text-slate-400 font-semibold">Branch / Stream</span>
                <p className="text-navy-900 dark:text-white font-bold">{book.branch}</p>
              </div>
            )}
            {book.isbn && (
              <div className="space-y-0.5">
                <span className="text-gray-500 dark:text-slate-400 font-semibold">ISBN</span>
                <div className="flex items-center gap-1.5 text-navy-900 dark:text-white font-mono font-bold">
                  <span className="truncate">{book.isbn}</span>
                  <button
                    onClick={copyIsbnToClipboard}
                    className="p-1 hover:text-brand-600 dark:hover:text-amber-400 transition-colors"
                    title="Copy ISBN"
                  >
                    {copiedIsbn ? <Check className="w-3 h-3 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-3 h-3" />}
                  </button>
                </div>
              </div>
            )}
            {book.publisher && (
              <div className="space-y-0.5">
                <span className="text-gray-500 dark:text-slate-400 font-semibold">Publisher</span>
                <p className="text-navy-900 dark:text-white font-bold truncate">{book.publisher}</p>
              </div>
            )}
            {book.edition && (
              <div className="space-y-0.5">
                <span className="text-gray-500 dark:text-slate-400 font-semibold">Edition</span>
                <p className="text-navy-900 dark:text-white font-bold">{book.edition}</p>
              </div>
            )}
            <div className="space-y-0.5">
              <span className="text-gray-500 dark:text-slate-400 font-semibold">Campus Area</span>
              <p className="text-navy-900 dark:text-white font-bold flex items-center gap-1 truncate">
            ...
            <div className="space-y-0.5">
              <span className="text-gray-500 dark:text-slate-400 font-semibold">Listed Date</span>
              <p className="text-navy-900 dark:text-white font-bold flex items-center gap-1">
          ...
          {book.listingType === 'SWAP' && book.swapPreferences && (
            <div className="p-4 rounded-2xl bg-accent-50 dark:bg-amber-950/30 border border-accent-200/80 dark:border-amber-800/40 space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-bold text-accent-900 dark:text-amber-300 uppercase tracking-wider">
                <Repeat className="w-3.5 h-3.5 text-accent-600 dark:text-amber-400" />
                <span>Owner's Swap Preference</span>
              </div>
              <p className="text-xs text-accent-800 dark:text-amber-200 leading-relaxed font-medium">
          ...
          {book.description && (
            <div className="space-y-1.5">
              <h3 className="font-serif text-base font-bold text-navy-900 dark:text-white">Textbook Description</h3>
              <p className="text-sm text-gray-600 dark:text-slate-300 leading-relaxed whitespace-pre-line bg-white dark:bg-[#132B32] p-4 rounded-2xl border border-cream-200 dark:border-[#26525E]">
                {book.description}
              </p>
            </div>
          )}
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-[#132B32] border border-cream-200 dark:border-[#26525E] shadow-soft flex items-center justify-between">
            <div className="flex items-center gap-3">
              {book.owner?.profileImage ? (
                <img
                  src={book.owner.profileImage}
                  alt={book.owner.name}
                  className="w-12 h-12 rounded-full object-cover border border-cream-300 dark:border-[#26525E]"
                />
              ) : (
                <div className="w-12 h-12 rounded-full bg-navy-800 dark:bg-amber-500 text-white dark:text-navy-950 flex items-center justify-center font-bold text-base">
              ...
              <div>
                <h4 className="text-sm font-bold text-navy-900 dark:text-white flex items-center gap-1.5">
                  ...
                  <ShieldCheck className="w-4 h-4 text-brand-600 dark:text-emerald-400" title="Verified Campus Student" />
                </h4>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-3">
            {isOwner ? (
              <div className="p-3.5 rounded-2xl bg-cream-200/80 dark:bg-[#0E2328] border border-cream-300 dark:border-[#26525E] text-xs font-semibold text-navy-800 dark:text-slate-300 flex-1 text-center">
                This is your active listing. You can manage incoming requests in your Requests tab.
              </div>
            ) : !isAvailable ? (
              <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/40 text-xs font-semibold text-amber-800 dark:text-amber-300 flex-1 text-center">
              ...
              <button
                ...
                className="flex-1 py-3.5 px-6 rounded-2xl bg-brand-600 hover:bg-brand-700 dark:bg-amber-500 dark:hover:bg-amber-600 text-white dark:text-navy-950 text-sm font-bold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
            ...
            <button
              ...
              className={`p-3.5 rounded-2xl border text-sm font-semibold transition-all flex items-center gap-2 ${
                inWishlist
                  ? 'border-rose-300 dark:border-rose-800 bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 shadow-sm'
                  : 'border-cream-300 dark:border-[#26525E] bg-white dark:bg-[#132B32] hover:bg-cream-100 dark:hover:bg-[#183B45] text-gray-700 dark:text-slate-200'
              }`}
      ...
      {relatedBooks.length > 0 && (
        <section className="pt-8 border-t border-cream-200 dark:border-[#1E434D] space-y-6">
          <div className="flex items-center justify-between">
            <div>
            ...
            <Link
              ...
              className="text-xs font-bold text-brand-600 dark:text-amber-400 hover:text-brand-700 dark:hover:text-amber-300 hover:underline"
            >
      ...
      <StudentReviewsSection bookId={book._id} />
